  ...
  (error: AxiosError) => {
    if (error.response?.status === 401) {
      ...
      if (!isAuthEndpoint && hadAuthHeader) {
        ...
        // Get current auth states before clearing
        const adminState = useAdminAuthStore.getState();
        const wasAdminAuthenticated = adminState.isAuthenticated;
        const customerToken = localStorage.getItem("token");
        const wasCustomerAuthenticated = !!customerToken;
        ...
        // Only show modal if user was actually authenticated in this session
        const wasActuallyLoggedIn = isAdminRequest ? wasAdminAuthenticated : wasCustomerAuthenticated;
        if (wasActuallyLoggedIn) {
